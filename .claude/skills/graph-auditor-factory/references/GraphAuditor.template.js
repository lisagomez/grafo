...
export class GraphAuditor {
  ...
  async detectarOrfanos({ pais, fecha } = {}) {
    ...
    // Índice O(R): clave de gasto → normas que le APLICA_A. Evita el escaneo por gasto.
    const normasPorGasto = new Map();
    for (const r of grafo.relaciones || []) {
      if (r.tipo !== 'APLICA_A') continue;
      const norma = normaById.get(r.fromVal);
      if (!norma) continue;
      const arr = normasPorGasto.get(r.toVal);
      if (arr) arr.push(norma); else normasPorGasto.set(r.toVal, [norma]);
    }

    const issues = [];
    for (const g of grafo.gastos || []) {
      const aplican = normasPorGasto.get(g.clave) || [];
      if (aplican.length === 0) {
  ...
  async validarConsistenciaVersiones({ pais } = {}) {
    ...
    const derogadas = new Set();
    // Índice O(R): id de norma → relaciones APLICA_A que salen de ella. Una sola pasada.
    const aplicaPorNorma = new Map();
    for (const r of rels) {
      if (r.tipo === 'DEROGA') derogadas.add(r.toVal);
      else if (r.tipo === 'APLICA_A') {
        const arr = aplicaPorNorma.get(r.fromVal);
        if (arr) arr.push(r); else aplicaPorNorma.set(r.fromVal, [r]);
      }
    }
    ...
    for (const n of grafo.normas || []) {
      ...
      for (const r of aplicaPorNorma.get(n.id) || []) {
